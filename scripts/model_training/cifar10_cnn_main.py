...
import utils
from utils import CIFAR_CNN100K, CIFAR_CNN47K, CIFAR_CNN25K
...
if variant == 0:
    model = CIFAR_CNN25K()
    model_name = f"cnn_params=025k_seed={str(seed).zfill(3)}"
elif variant == 1:
    model = CIFAR_CNN47K()
    model_name = f"cnn_params=047k_seed={str(seed).zfill(3)}"
elif variant == 2:
    model = CIFAR_CNN100K()
    model_name = f"cnn_params=100k_seed={str(seed).zfill(3)}"
