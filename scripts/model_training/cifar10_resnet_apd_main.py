...
# USE A GPU FOR RESNETS IF POSSIBLE!
...
# command-line arguments: ONLY NEED THE INDEX TELLING US WHICH variant + seed we need to re-run!
dataset = "CIFAR10"
variant, seed = tuple(pd.read_csv("cifar_fails.csv").loc[int(sys.argv[1])][["var_id", "seed"]].values) # 65 settings!
...
# based on "variant", instantiate our model + create our filename
if variant == 0:
    model = resnet20(); model.to(device)
    model_name = f"resnet_variant=20_seed={str(seed).zfill(3)}"
elif variant == 1:
    model = resnet32(); model.to(device)
    model_name = f"resnet_variant=32_seed={str(seed).zfill(3)}"
elif variant == 2:
    model = resnet44(); model.to(device)
    model_name = f"resnet_variant=44_seed={str(seed).zfill(3)}"
